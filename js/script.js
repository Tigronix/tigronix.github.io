$(document).ready(function () {
    ...
    $(".rooms_slider").owlCarousel({
     ...
     //slide_slider
    $('.choice__room').click(function () {
        $('.slide_that').addClass('slide_lefts').addClass('flex_show');
    });
    
    jQuery(function($){
        $(document).mouseup(function (e){ // событие клика по веб-документу
            var div = $(".slide_that"); // тут указываем ID элемента
            if (!div.is(e.target) // если клик был не по нашему блоку
                && div.has(e.target).length === 0) { // и не по его дочерним элементам
                div.removeClass('flex_show'); // скрываем его
            }
        });
    });
    //row_hover
...
$(document).ready(function () {
    ...
    $('.header__open').click(function () {
        ...
        $('.mob_phone2').addClass('header__on').removeClass('move_away').addClass('move_left_header').fadeIn(400);
